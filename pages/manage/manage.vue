		...
		methods: {
			async fetchAllUsers() {
				try {
					...
					const response = await axios.post('/user/admin/list', {}, // 空请求体，因为接口不接收参数
						{
							headers: {
					...
					if (response.data.code === 200) {
						this.userList = response.data.data.map(user => ({
							...
							email: user.email || '无',
							registerTime: new Date(user.createdAt).toLocaleString(),
			...
			async searchUsers() {
				try {
					// 转换搜索参数
					const params = {
						username: this.userSearchParams.username,
						status: this.userSearchParams.status,
						startDate: this.userSearchParams.startDate,
						endDate: this.userSearchParams.endDate
					};
					...
					if (res.code === 200) {
						this.userList = res.data.map(user => ({
							...
							registerTime: new Date(user.createdAt).toLocaleString(),
							lastLogin: user.lastLogin ? new Date(user.lastLogin).toLocaleString() : '从未登录',
							status: user.enabled ? '活跃' : '禁用'
						}));

						uni.showToast({
							title: `找到 ${this.userList.length} 个用户`,
							icon: 'success',
							duration: 2000
						});
					}
